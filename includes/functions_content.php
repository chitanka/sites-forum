...
function make_clickable($text, $server_url = false, $class = 'postlink')
{
	...
	if (!is_array($magic_url_match) || $static_class != $class)
	{
		...
		$magic_url_replace[] = fn($m) => make_clickable_callback(MAGIC_URL_LOCAL, $m[1], $m[2], $m[3], $local_class);

		// matches a xxxx://aaaaa.bbb.cccc. ...
		$magic_url_match[] = '#(^|[\n\t (>.])(' . get_preg_expression('url_inline') . ')#i';
		$magic_url_replace[] = fn($m) => make_clickable_callback(MAGIC_URL_FULL, $m[1], $m[2], '', $class);

		// matches a "www.xxxx.yyyy[/zzzz]" kinda lazy URL thing
		$magic_url_match[] = '#(^|[\n\t (>])(' . get_preg_expression('www_url_inline') . ')#i';
		$magic_url_replace[] = fn($m) => make_clickable_callback(MAGIC_URL_WWW, $m[1], $m[2], '', $class);

		// matches an email@domain type address at the start of a line, or after a space or after what might be a BBCode.
		$magic_url_match[] = '/(^|[\n\t (>])(' . get_preg_expression('email') . ')/i';
		$magic_url_replace[] = fn($m) => make_clickable_callback(MAGIC_URL_EMAIL, $m[1], $m[2], '', '');
	}

	return preg_replace_callback_array(array_combine($magic_url_match, $magic_url_replace), $text);
}
...
*/
function truncate_string($string, $max_length = 160, $max_store_length = 255, $allow_reply = false, $append = '') // borislav: increased max_length
{
	$chars = array();
