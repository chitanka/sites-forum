...
class CustomAJAXChat extends AJAXChat {
	...
	function replaceCustomTemplateTags($tag, $tagContent) {
		...
		switch($tag) {
			...
			case 'USER_COLORS': // borislav
				return $this->getUserStyleClasses();
	...
	function getUserStyleClasses() {
		...
		$sql = 'SELECT
					...
					FROM
						'.USERS_TABLE.'
					WHERE
						group_id != 6/*bots*/ AND user_colour != ""';
		$result = $db->sql_query($sql);

		$classes = array();
		while ($row = $db->sql_fetchrow($result)) {
			$classes[] = ".$row[username] {color: #$row[user_colour] !important}";
		}
		$db->sql_freeresult($result);
